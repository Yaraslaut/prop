...
struct AxisUnits
{
    ...
    l_unit getCoord(index i) const { return _min + l_unit(dx * i); }
    KOKKOS_INLINE_FUNCTION
    index getIndex(l_unit x) { return static_cast<index>((x - _min) / dx); }

    static std::tuple<int, int> getOffsetAndSize(AxisUnits<T> big, AxisUnits<T> small)
    {
...
class Geometry2D
{
  ...
  public:
...
#ifdef USE_SPDLOG
        spdlog::info("Creation of geometry with size {}x{}\n", _x._N, _y._N);
        spdlog::info("And step dx: {}  dy : {}\n", _x.dx, _y.dx);
#endif
    };
    ...
    std::tuple<int, int, int, int> getProperties(Box box)
    {
        ...
        auto xProps = Axis::getOffsetAndSize(this->_x, box._x);
        int x_offset = std::get<0>(xProps);
        int x_size = std::get<1>(xProps);

        auto yProps = Axis::getOffsetAndSize(this->_y, box._y);
