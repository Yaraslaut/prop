...
struct IsotropicMedium
{
    ...
    IsotropicMedium(Axis x , Axis y, double eps, double sigm, double mu): _epsilon(eps), _mu(mu), _sigma(sigm), _box(x,y) {};
    ...
    void Propagator(double time, double time_step, Grid2DRectangular global_field, Geometry2D& global_geometry)
    {
        ...
        Kokkos::parallel_for(policy, KOKKOS_LAMBDA(const int& iinit, const int& jinit){
                ...
                if(entity_ind(i,j)==entity_id)
                {
                    ...
                    auto C_ezhx { time_step / (1.0 + (sigma * time_step) / (2.0 * epsilon)) / (epsilon * space_step) };
                    auto C_ezhy { time_step / (1.0 + (sigma * time_step) / (2.0 * epsilon)) / (epsilon * space_step) };

                    Ez(i, j) = C_eze * Ez(i, j) + C_ezhx * (Hy(i, j) - Hy(i - 1, j)) - C_ezhy * (Hx(i, j) - Hx(i, j - 1));
                }
            });
    }
};
